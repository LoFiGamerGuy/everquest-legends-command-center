...
2. **Diff:** first **refresh the beta baseline** — regenerate `BETA_FAMILY_COUNTS` (`packages/log-parser/src/baselines/eql-beta-2026-07.ts`) from the measured beta corpus, not the coverage-sample fixtures, so the family shares are frequency-representative. The shipped fixture-derived baseline is only reliable for detecting a family that *fully disappears*; diffing a real corpus against it would false-positive unchanged-but-rare families. Then run `driftReport` vs the refreshed baseline → the ranked list of changed families + new unknown shapes.
