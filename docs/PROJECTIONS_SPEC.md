...
- **Close:** the encounter closes ONLY when a **later event** proves the idle `timeout` has elapsed since `ended_ts`: `group` scale ⇒ **15 s**, `raid` scale ⇒ **2 min** (rumstil FightTracker precedent). Closing is purely event-driven — there is no forced close at end-of-pass, so a **trailing still-`active` encounter is EXPECTED** at the head of a finite event set (it mirrors the trailing still-open session). This is what keeps incremental == rebuild exact (§9.2): a batch boundary landing inside a live window never changes the outcome. A caller that knows a log is complete may invoke the OPTIONAL `finalizeEncounters(db, asOfTs?)` (§8) to close still-active encounters whose window has elapsed as of `asOfTs`; this is a deliberate terminal action, NOT part of the incremental == rebuild core (a later rebuild re-derives the trailing encounter as `active`).
...
- `finalizeEncounters(db, asOfTs?)` — OPTIONAL terminal close for a completed log: closes still-`active` encounters whose `ended_ts + timeout(scale) < asOfTs` (or all active when `asOfTs` is omitted). Deliberately NOT part of the incremental == rebuild core (a subsequent rebuild re-derives the trailing encounter as `active`).
